...
export default function Sidebar({ collapsed, setCollapsed, mobileOpen, setMobileOpen }) {
  ...
  const content = (
    <div className="h-full flex flex-col bg-neutral-950/95 text-neutral-100">
      ...
      {!collapsed && (
        <div className="px-3 pb-2">
          <div className="grid grid-cols-3 gap-2">
            <NavLink to="/loads" className="text-xs text-center py-2 rounded-lg bg-neutral-800/70 hover:bg-neutral-800 transition">+ Load</NavLink>
            <NavLink to="/drivers" className="text-xs text-center py-2 rounded-lg bg-neutral-800/70 hover:bg-neutral-800 transition">+ Driver</NavLink>
            <NavLink to="/trucks" className="text-xs text-center py-2 rounded-lg bg-neutral-800/70 hover:bg-neutral-800 transition">+ Truck</NavLink>
          </div>
        </div>
      )}
      ...
      <div className={cx("border-t border-neutral-800/70", collapsed ? "p-2" : "p-3")}>
        <button className={cx("w-full flex items-center gap-2 rounded-lg hover:bg-neutral-800/70", collapsed ? "p-2" : "p-2.5")} title="Account">
          <img src="https://api.dicebear.com/9.x/identicon/svg?seed=mark" alt="User" className="size-7 rounded-md bg-neutral-800" />
          {!collapsed && (
            <>
              ...
              <span className="ml-auto text-xs px-2 py-0.5 rounded-md bg-emerald-700/30 text-emerald-200">Online</span>
            </>
          )}
        </button>
      </div>
  ...
  return (
    <>
      {/* Desktop */}
      <aside className={cx("hidden md:block sticky top-0 h-screen border-r border-neutral-800/70", collapsed ? "w-[4.25rem]" : "w-64")}>
        {content}
      </aside>

      {/* Mobile drawer */}
      <div className={cx("md:hidden fixed inset-0 z-50 transition", mobileOpen ? "pointer-events-auto" : "pointer-events-none")}>
        <div className={cx("absolute inset-0 bg-black/50 transition-opacity", mobileOpen ? "opacity-100" : "opacity-0")} onClick={() => setMobileOpen(false)} />
        <aside className={cx("absolute left-0 top-0 h-full w-72 shadow-2xl border-r border-neutral-800/70 transition-transform", mobileOpen ? "translate-x-0" : "-translate-x-full")}>
          {content}
        </aside>
      </div>
    </>
